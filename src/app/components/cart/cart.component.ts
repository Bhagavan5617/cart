...
export class CartComponent implements OnInit {
  ...
  getCartProducts() {
    this.cartService.getCartItems().subscribe((data: any) => {
      this.totalCartValue = data.reduce((total:any,item:any)=> total += item.cartValue , 0)  
  ...
  decreasedCartItemObject(product: any) {
    ...
    const getIndex = this.cartProducts.findIndex((item: any) => item.id === product.id);
    if (getIndex === -1) {
    ...
    if (this.cartProducts[getIndex].cartQuantity > 1) {
      this.cartProducts[getIndex].cartValue =
        (this.cartProducts[getIndex].cartQuantity - 1) * this.cartProducts[getIndex].price;
